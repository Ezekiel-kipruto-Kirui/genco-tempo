...
exports.health = exports.cacheStats = exports.batchDelete = exports.remove = exports.setRecord = exports.update = exports.create = exports.analysisCore = exports.analysisSummary = exports.authVerify = exports.queryEndpoint = exports.record = exports.data = void 0;
...
exports.data = functions.https.onRequest(withCors(async (req, res) => {
    ...
    if (programme || orderBy) {
        ...
        try {
            let q = db.ref(collectionPath).orderByChild(orderField);
            if (programme) {
            ...
            if (programme) {
                ...
                if (snap2?.exists()) {
                    ...
                    records2.forEach((r) => { if (!existingIds.has(r.id))
                        records.push(r); });
                }
            }
        }
...
exports.queryEndpoint = functions.https.onRequest(withCors(async (req, res) => {
    ...
    if (programmes && programmes.length > 0) {
        ...
        const snap = await db.ref(collectionPath).once("value");
        records = snapToRecords(snap);
        const upperSet = new Set(programmes.map((p) => p.trim().toUpperCase()));
 ...
 */
exports.analysisSummary = functions.https.onRequest(withCors(async (req, res) => {
    ...
    filteredTraining.forEach((t) => {
        ...
        if (Array.isArray(benes))
            benes.forEach((b) => { if (b.id)
                trainedSet.add(b.id); });
    });
    result.totalTrainedFarmers = trainedSet.size;
    cache.set(cacheKey, result, computeVersion([result]));
    res.json(result);
}));
/**
 * GET /api/analysis-core — Full analytics data for dashboard (cached, heavy)
 */
exports.analysisCore = functions.https.onRequest(withCors(async (req, res) => {
    if (req.method !== "GET") {
        res.status(405).json({ error: "Method not allowed" });
        return;
    }
